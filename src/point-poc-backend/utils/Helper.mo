    ...
    public func getRecoveryId(
        ...
        label L for(i in Iter.range(0, 2)) {
            ...
            switch(Ecdsa.recover_with_context(
                ...
                case (#ok(key)) {
                    let compressed_pubkey = key.serialize_compressed();
                    let computed_address = publicKeyToAddress(compressed_pubkey);
                    switch(computed_address) {
                        case(#ok(addr)) {
                            if (AU.fromText(addr) == address) {
